/**
 * DeskTradingAgent — single-shot structured completion over a pre-fetched context.
...
export interface DeskTradingAgentConfig {
  ...
  /** Injected caller for tests (bypasses real HTTP). */
  callLlm?: (
...
function providerOrder(
  ...
  // Only providers with non-empty API keys
  ...
  return {
    async run(context: DeskAgentContext): Promise<DeskAgentRunResult> {
      ...
      if (context.policy.paused || context.policy.killSwitchArmed) {
        ...
        // Kill switch still blocks risk-increasing; force defend only when not kill-armed
      ...
      if (providers.length === 0) {
        ...
        proposal = applyForceDefendOverride(proposal, {
          ...
          hfCritical: context.policy.hfCritical,
          paused: context.policy.paused,
          forceDefendEnabled: forceDefend,
        });
        if (proposal.action !== "defend") {
          proposal = applyForceMaintenanceOverride(
            proposal,
      ...
      for (const provider of providers) {
        ...
        try {
          const call =
            config.callLlm ??
            ((p, c, prompt, signal, sys) => LLM_PROVIDER_CALLERS[p](c, prompt, signal, sys));

          const raw = await call(provider, cfg, userPrompt, controller.signal, systemInstruction);
          ...
          let proposal: DeskAgentProposal = parsed.proposal;
          if (!parsed.ok) {
            errors.push(`${provider}:${parsed.error}`);
            // try next provider on schema fail
            continue;
          }

          proposal = applyMinConfidence(proposal, minConfidence);
          proposal = applyForceDefendOverride(proposal, {
            healthFactor: context.mark.healthFactor,
            hfCritical: context.policy.hfCritical,
            paused: context.policy.paused,
