    ...
    async publishDigest(digest) {
      ...
      try {
        ...
        if (allowRegistryWrite) {
          ...
          let registryResult: RegistryPublishResult;
          try {
            registryResult = await registryService.publishDigest(
              contentHash,
            ...
            const failMessage =
              error instanceof Error ? error.message : "Registry write threw";
            failures.push(`Registry: ${failMessage}`);
            registryResult = { success: false, errorMessage: failMessage };
          }
          ...
          if (registryResult.success && registryResult.txHash) {
            ...
            if (!failures.some((f) => f.includes(failMessage))) {
              failures.push(`Registry: ${failMessage}`);
            }
            await digestRepo.updateRegistryMetadata(digest.id, { contentUri, contentHash });
            if (execLogRepo) {
        ...
        try {
          ...
          const msg = error instanceof Error ? error.message : "SMTP send threw";
          failures.push(`SMTP: ${msg}`);
          smtpResult = { success: false, recipientsReached: 0, errorMessage: msg };
        }
        ...
        // Never leave a digest stuck in draft/queued after entering publish.
        const message =
          error instanceof Error ? error.message : "Unexpected publication failure";
        failures.push(message);
        if (!statusWritten) {
          try {
            await digestRepo.updatePublicationStatus(digest.id, "failed", publishedAt);
            statusWritten = true;
          } catch (statusError) {
            console.error(
              "[digest-publication] failed to mark digest failed after throw:",
