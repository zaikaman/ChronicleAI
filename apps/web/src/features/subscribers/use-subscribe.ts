...
export function useSubscribe(): UseSubscribeResult {
  ...
  // Read the latest wallet context (stub → live) from a ref so async closures
  // created before the wallet stack loads can still sign after connecting.
  ...
  const subscribe = useCallback(async (email: string): Promise<boolean> => {
    ...
    try {
      ...
      void attributeReferralOnConnect(payer, walletRef.current.signMessage);
      ...
      try {
        ...
        setMessage(
          err instanceof Error
            ? err.message
            : `Switch your wallet to ${walletRef.current.targetChain.name} to continue.`,
        );
        return false;
      }
