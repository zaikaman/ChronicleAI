...
export function PaymentRequiredModal({
  ...
  return (
    <dialog
      ref={dialogRef}
      aria-label={passRequired ? "Unlock report or get Chronicle Pass" : "Payment required"}
      onCancel={(event) => {
      ...
      {showPaymentPanel ? (
        ...
        >
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              {passRequired ? (
                <StatusBadge label="Included with Chronicle Pass" variant="info" />
              ) : (
          ...
          <p className="text-sm leading-relaxed text-muted-foreground">
            Buy this report on its own, or get Chronicle Pass for every deep dive and the full
            archive.
          </p>
          {itemTitle ? (
            ...
            {passRequired ? (
              <span className="ml-auto text-right text-[11px] leading-tight text-muted-foreground">
                ...
                <span className="font-semibold text-accent">
                  {CHRONICLE_PASS_PRICE_USDC.toFixed(2)} USDC/mo
                </span>
              </span>
            ) : null}
          </div>
            ...
            {passRequired ? (
              <>
                <Link
                  ...
                  className="w-full rounded-xl border border-border bg-background px-5 py-3 text-center text-sm font-semibold text-foreground transition-colors hover:border-accent/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  data-testid="pass-upgrade-cta-link"
                >
                  Get Chronicle Pass — {CHRONICLE_PASS_PRICE_USDC.toFixed(2)} USDC/month
                </Link>
                <p className="text-center text-[11px] leading-relaxed text-muted-foreground">
                  One subscription unlocks every deep dive and the full archive. Renewals stay
                  wallet-authorized.
                </p>
              </>
            ) : (
          ...
          <p className="mt-5 text-center text-xs leading-relaxed text-muted-foreground">
            Wallet checkout is for people. Automated agents should use the per-item API checkout.
          </p>
        </div>
      )}
    </dialog>
